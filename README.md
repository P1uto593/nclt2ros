This repository is the Python 3 version of the https://github.com/bierschi/nclt2ros repository. If there is any infringement, it will be deleted
The original repository can run normally using python2 in Ubuntu 18, but the default python version is 3.8 in Ubuntu 20, which makes the original repository script unusable.

If you encounter issues with running permissions
chmod +x your_dir/catkin_ws/src/nclt2ros/scripts/nclt2downloader

### Warehouse modification description (generated by AI)

#### **Core issue: Python 2 → Python 3 compatibility**

1.  **`long` type does not exist**
    *   **Error**: `NameError: name 'long' is not defined`
    *   **Solution**: Replace `long()` with `int()`. Python 3's `int` can handle integers of arbitrary size.

2.  **String and bytes (str vs. bytes) mismatch**
    *   **Error**: `'utf-8' codec can't decode byte...` and `if x_str == ''` comparison fails.
    *   **Solution**:
        *   Read files in binary mode: `open(file, 'rb')`.
        *   Use `np.frombuffer()` to parse binary data, not `np.fromstring()`.
        *   Compare for end-of-file with `b''` instead of `''`.

3.  **Numpy method deprecated**
    *   **Error**: The `.tostring()` method may trigger warnings or errors.
    *   **Solution**: Replace `ndarray.tostring()` with `ndarray.tobytes()`.

4.  **PointCloud2 message field type incorrect**
    *   **Error**: `SerializationError: field width must be unsigned integer type`
    *   **Solution**: Ensure `pc2_msg.width` and `pc2_msg.height` are assigned integers. Use integer division `//` instead of floating-point division `/`.

5.  **Multiple inheritance MRO conflict**
    *   **Error**: `TypeError: Cannot create a consistent method resolution order (MRO)`
    *   **Solution**: Use **composition** instead of multiple inheritance. Create a main class that contains instances of other functional classes.

6.  **Dictionary method `iteritems()` does not exist**
    *   **Error**: `AttributeError: 'dict' object has no attribute 'iteritems'`
    *   **Solution**: Replace `dict.iteritems()` with `dict.items()`.
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# NCLT2ROS
